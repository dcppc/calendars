...
def create_calendar():
    ...
    try:
        calendar = {
            'summary': 'Update Event Test Calendar',
...
def populate_calendar():
    ...
    try:
        ...
        # utc = eastern + 4
        # so this is 1:30 - 2:30 PM eastern
...
def update_calendar():
    ...
    calendar_id = calendar_id.strip()
    print("Updating events on calendar %s"%(calendar_id))
    ...
    for calendar_list_entry in calendar_list['items']:
        ...
        if cal_name == 'Update Event Test Calendar':
            ...
            for events_list_entry in events_list['items']:
                event = events_list_entry
                break

    dts = parse(event['start']['dateTime'])
    dte = parse(event['start']['dateTime'])

    # This should change the original 1:30 - 2:30 eastern to 4:30 - 5:30 eastern
    dts = dts + datetime.timedelta(hours=3)
    dte = dte + datetime.timedelta(hours=3)

    event['start']['dateTime']  = dts.isoformat("T")
    event['end']['dateTime']    = dte.isoformat("T")
